...
AppArmor writes denials to syslog. In a container, read it directly:
...
```
... kernel: audit: type=1400 ...: apparmor="DENIED" operation="connect" profile="snap.inspire.inspire" ...
```

Key fields:
- `apparmor="DENIED"` — the action was blocked
- `profile="snap.inspire.inspire"` — the sandboxed process (`snap.<name>.<app>`)
- `operation="connect"` — a network socket call was attempted

If syslog is empty, try the journal instead:
...
> 📖 **Concept — AppArmor**  
> AppArmor is a Linux Security Module that enforces per-process access control profiles. Snapd generates an AppArmor profile for every strictly confined snap. Any call not explicitly allowed in the profile is denied and logged to syslog.  
